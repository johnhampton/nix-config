...
let
  ...
  darwinBuild = ''
    ${pkgs.nixFlakes}/bin/nix build --no-link ".#darwinConfigurations.Ava.system" --experimental-features "flakes nix-command" --show-trace
  '';

  darwinInstall = pkgs.writeShellScriptBin "darwinInstall" ''
    ${systemSetup}
    ${darwinBuild}
    sudo $(${pkgs.nixFlakes}/bin/nix path-info ".#darwinConfigurations.Ava.system" --experimental-features "flakes nix-command")/activate
  '';
